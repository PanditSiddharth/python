...
async function pyStarter(bot: any, ctx: any) {
  try {
    ...
    if (!fs.existsSync(`./py/files/${cmp + id + cmp}.ts`)) {
      const data = fs.readFileSync('./py/py/py.ts', 'utf8');
      const modifiedData = data.replace(/pyyoyopy/g, cmp + id + cmp);
      fs.writeFileSync(`./py/files/${cmp + id + cmp}.ts`, modifiedData);
      setTimeout(() => {
        try {
          fs.unlinkSync(`./py/files/${cmp + id + cmp}.ts`)
          if (`flag[py${id}]`)
            delete flag[cmp + id];
    ...
    const moduleExports = require(`./py/files/${cmp + id + cmp}`);
